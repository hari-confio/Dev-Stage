...
EmberAfStatus emberAfClusterSpecificCommandParse(EmberAfClusterCommand *cmd)
{
  ...
  if (cmd->direction == (uint8_t)ZCL_DIRECTION_SERVER_TO_CLIENT
    ...
    case ZCL_POLL_CONTROL_CLUSTER_ID:
      ...
      break;
    case ZCL_CUSTOM_END_DEVICE_CONFIGURATION_CLUSTER_ID:
      result = emberAfCustomEndDeviceConfigurationClusterServerCommandParse(cmd);
      break;
    default:
      // Unrecognized cluster ID, error status will apply.
      break;
...
// Cluster: custom end device configuration Cluster, server
EmberAfStatus emberAfCustomEndDeviceConfigurationClusterServerCommandParse(EmberAfClusterCommand *cmd)
{
  bool wasHandled = false;
  if (cmd->mfgSpecific) {
    if (cmd->mfgCode == 0x1002 && cmd->commandId == ZCL_COMMAND_ONE_COMMAND_ID) {
      ...
      wasHandled = emberAfCustomEndDeviceConfigurationClusterCommandOneCallback(argOne);
    }
  }
  return status(wasHandled, true, cmd->mfgSpecific);
}
