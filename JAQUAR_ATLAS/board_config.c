...
SSwTimer time_config_addr;
SSwTimer send_one_time_cmd;
SSwTimer knob_in_addr_mode;
...
uint32_t Board_HW_Init(void)									//GPIO INIT
{
	...
	//AppTimerRegister(&check_time, false, check_time_CB);

	GPIOINT_CallbackRegister(LEARN_RESET_BTN_PIN, Interrupt_CB);
	AppTimerRegister(&time_config_addr, false, time_config_addr_CB);
	//send_one_time_cmd
	AppTimerRegister(&send_one_time_cmd, false, send_one_time_cmd_CB);
	AppTimerRegister(&knob_in_addr_mode, false, knob_in_addr_mode_CB);
