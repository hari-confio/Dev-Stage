...
def S37():
    ...
    if ("gcc" == "iar" or "gcc" == ""):
        ...
        GCC_TOOLCHAIN_PATH = shortPath(r"file:/C:/SiliconLabs/SimplicityStudio/v5/developer/toolchains/gnu_arm/12.2.rel1_2023.7/".replace(
            "file:", "").replace("%20", " "))
...
def EBL():
    ...
    print (" ")
    EBL_COMMAND = START_FLAGS + shortPath(r"C:\SiliconLabs\SimplicityStudio\v5\developer\adapter_packs\commander\commander.exe").replace(
        " ", "\ ") + " ebl create " + EBL_FILE + " --app " + S37_BINARY + " --device EFR32MG21A010F1024IM32 " + CONVERT_FLAGS
...
def GBL():
    ...
    GBL_COMMAND = START_FLAGS + shortPath(r"C:\SiliconLabs\SimplicityStudio\v5\developer\adapter_packs\commander\commander.exe").replace(
        " ", "\ ") + " gbl create " + GBL_FILE + " --app " + S37_BINARY + " --device EFR32MG21A010F1024IM32"
...
def OTA():
    if not '"%PROJECT_DIR%\..\\..\\..\\..\\..\\SiliconLabs\\SimplicityStudio\\v5\\developer\\sdks\\gecko_sdk_suite\\v3.2\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 101 --manuf-id 0x1002 --image-type 0005 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL CTSMZB_8PB_AC_V101"':
        sys.exit(0)
    print (" ")
    ...
    IMAGE_BUILDER = '"%PROJECT_DIR%\..\\..\\..\\..\\..\\SiliconLabs\\SimplicityStudio\\v5\\developer\\sdks\\gecko_sdk_suite\\v3.2\\protocol\\zigbee\\tool\\image-builder\\image-builder-windows.exe" --create "%TARGET_BPATH%.ota" --version 101 --manuf-id 0x1002 --image-type 0005 --tag-id 0x0000 --tag-file "%TARGET_BPATH%.gbl" --string "EBL CTSMZB_8PB_AC_V101"'.replace("\t", "\\t")
