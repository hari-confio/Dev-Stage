...
for(int i=0; i<8; i++)
  {
    if(Rx_Buff[i] == MACID[i])
      {
        mac_match_cnt++;
        if(mac_match_cnt == 7 && Rx_Buff[8] == 0x02) //2pb
          {
            // Proceed for further processing
            if(Rx_Buff[9] == 0x01) //BTN-1, 2, 3, 4
              {
...
//                     emberAfCorePrintln("--Response--BTN - %d\n", 1);
              ...
              //  if(Rx_Buff[10] != 0x04 || Rx_Buff[10] != 0x0D)
                //  {
                  for(int led = 1; led <=4; led++) // Toggle upto 4(1-4) LED's
                    {
                      set_led(led, Rx_Buff[11]);
                    }
                 // }

              }
            else if(Rx_Buff[9] == 0x02) //BTN-5, 6, 7, 8
              {
                 if(Rx_Buff[10] == 0x01)
                 {
                 ...
                 for(int led = 5; led <=8; led++) // Toggle upto 4(5-8) LED's
                   {
                     set_led(led, Rx_Buff[11]);
                   }
...
//    emberEventControlSetActive(closeNetworkEventControl);
  ...
  // Conversion to 2PB ASTRID

  else if(active_button == 1 || active_button == 2 || active_button == 3 || active_button == 4)
    {
      active_button = 1;
  ...
  if(active_button == 1 && active_count == 4)
    ...
    else if (active_button == 1 || active_button == 50 || active_button == 60 || active_button == 70 || active_button == 80)
    {
        ...
        else if (active_button == 50 || active_button == 60 || active_button == 70 || active_button == 80) // 50 indicates button long hold action
        {
...
//        }
    ...
    else if(active_button == 2 || active_button == 90 || active_button == 100 || active_button == 110 || active_button == 120)
      {
        ...
        else if(active_button == 90 || active_button == 100 || active_button == 110 || active_button == 120)
          {
            emberAfCorePrintln("---> Button %d Long Held\n", 2);
           // ledState2Long = !ledState2Long;
...
//    0x11,                       // Length
//    0x43, 0x6F, 0x6E, 0x66, 0x69, 0x6F, 0x3A, 0x43, 0x54, 0x38, 0x50, 0x42, 0x52, 0x53, 0x34, 0x38, 0x35,//(Confio:CT8PBRS485)
