...
void handleSwitchEvent1(uint8_t endpoint)
{

  EVENT_APP event_to_enqueue = EVENT_EMPTY;  // Default event
//  EVENT_SYSTEM event_to_enqueue1 = EVENT_SYSTEM_RESET;  // Default event
     switch (endpoint)
    {
        case 1:
          //  event_to_enqueue = EVENT_APP_TOGGLE_LED;
            DPRINTF("  \n IN SW1  \n");
            break;
        case 2:
          DPRINTF("  \n IN SW2  \n");

        //  event_to_enqueue = EVENT_APP_TOGGLE_LED2;
            break;
        case 3:
          DPRINTF("  \n IN SW3  \n");

      //      event_to_enqueue = EVENT_APP_TOGGLE_LED3;
            break;
        case 4:
          DPRINTF("  \n IN SW4  \n");

        //    event_to_enqueue = EVENT_APP_TOGGLE_LED4;
            break;
        case 5:
          DPRINTF("  \n IN SW5  \n");

       //     event_to_enqueue = EVENT_APP_TOGGLE_LED5;
            break;
        case 6:
          DPRINTF("  \n IN SW6  \n");

            //event_to_enqueue = EVENT_APP_TOGGLE_LED6;
            break;
        default:
        ...
        if (event_to_enqueue != EVENT_EMPTY)
        {
          //  zaf_event_distributor_enqueue_app_event(event_to_enqueue);
        }
}
...
static void actuator_callback(s_Actuator * p_actuator)
{
  ...
  // Print the endpoint associated with this actuator
  //   DPRINTF("\nEndpoint IS : %d \n", p_switch->endpoint);
  //   handleSwitchEvent1(p_switch->endpoint);

  uint8_t current_value = ZAF_Actuator_GetCurrentValue(p_actuator);

   //uint8_t target_value = ZAF_Actuator_GetTargetValue(p_actuator);
...
if(endpoint==1)
 ...
 //   zaf_event_distributor_enqueue_app_event_from_isr(APP_EVENT_SCENE_1);
    knx_frame_rem(true, false, false);  // Set btnpress1 true
...
if(endpoint==2)
  {
    ...
    knx_frame_rem(false, true, false);  // Set btnpress1 true

  }
if(endpoint==3)
    {
    knx_frame_rem(false, false,true);  // Set btnpress1 true
