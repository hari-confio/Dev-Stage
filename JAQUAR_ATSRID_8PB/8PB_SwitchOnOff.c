...
static uint8_t ep1234_noSec_InclNonSecure[] =
{
  COMMAND_CLASS_ZWAVEPLUS_INFO,
  COMMAND_CLASS_SWITCH_BINARY_V2,
  COMMAND_CLASS_ASSOCIATION,
...
static uint8_t ep1234_noSec_InclSecure[] =
{
  COMMAND_CLASS_ZWAVEPLUS_INFO,
  COMMAND_CLASS_SUPERVISION,
  COMMAND_CLASS_SECURITY_2
};
static uint8_t ep1234_sec_InclSecure[] =
{
  COMMAND_CLASS_SWITCH_BINARY_V2,
  COMMAND_CLASS_ASSOCIATION,
  COMMAND_CLASS_ASSOCIATION_GRP_INFO,
...
/*
static uint8_t ep5_noSec_InclNonSecure[] =
{
  COMMAND_CLASS_ZWAVEPLUS_INFO,
...
static uint8_t ep5_noSec_InclSecure[] =
{
  COMMAND_CLASS_ZWAVEPLUS_INFO,
  COMMAND_CLASS_SUPERVISION,
  COMMAND_CLASS_SECURITY_2
...
static uint8_t ep5_sec_InclSecure[] =
{
  COMMAND_CLASS_CENTRAL_SCENE,
  COMMAND_CLASS_ASSOCIATION,
  COMMAND_CLASS_ASSOCIATION_GRP_INFO,
    ...
    //Endpoint 1
      {
        ...
        {
          {ep1234_noSec_InclNonSecure, sizeof(ep1234_noSec_InclNonSecure)},
          {{ep1234_noSec_InclSecure, sizeof(ep1234_noSec_InclSecure)}, {ep1234_sec_InclSecure, sizeof(ep1234_sec_InclSecure)}}
        }
      },
...
ApplicationInit(EResetReason_t eResetReason)
{
  ...
  DPRINT("\n\n--------------------------------\n");
  DPRINT("Z-Wave Sample App: JAQUAR ASTRID 8PB \n");
...
ApplicationTask(SApplicationHandles* pAppHandles)
{
  ...
  //AppTimerRegister(&led_Timer, true, led_Timer_CB);
 // InitializeButton_rx_Timer();
...
AppStateManager(EVENT_APP event)
{
 // DPRINTF("AppStateManager St: %d, Ev: %d\r\n", currentState, event);
  ...
  if ((APP_EVENT_HARD_RESET == (APP_BUTTON_EVENT)event ) ||
  ...
  switch(currentState)
  {
    ...
    case STATE_APP_IDLE:
      ...
      if (APP_EVENT_INCLUSION == (APP_BUTTON_EVENT)event ||
...
//========================
      if(APP_EVENT_PB_UP == (APP_BUTTON_EVENT)event)
          {
             //TimerStart(&led_Timer,500);
             //ZAF_EventHelperEventEnqueueFromISR(APP_EVENT_INCLUSION);
          }
         if(APP_EVENT_SW1_DOWN == (APP_BUTTON_EVENT)event)
          {
             DPRINT("APP_EVENT_SW1_DOWN\n");
             createFrame(1);
            set_proximity_level();
         ...
         if(APP_EVENT_SW1_UP == (APP_BUTTON_EVENT)event)
         {
            DPRINT("APP_EVENT_SW1_UP\n");
            set_proximity_level();
            //Set_Relay(1, 1);
         }

          if(APP_EVENT_SW2_DOWN == (APP_BUTTON_EVENT)event)
          {
            DPRINT("APP_EVENT_SW2_DOWN\n");
            set_proximity_level();
            createFrame(2);
           ...
           if(APP_EVENT_SW2_UP == (APP_BUTTON_EVENT)event)
          {
             DPRINT("APP_EVENT_SW2_UP\n");
             set_proximity_level();
             // Set_Relay(2, 1);
          ...
          if(APP_EVENT_SW3_DOWN == (APP_BUTTON_EVENT)event)
          {
             DPRINT("APP_EVENT_SW3_DOWN\n");
             set_proximity_level();
             createFrame(3);
          ...
          if(APP_EVENT_SW3_UP == (APP_BUTTON_EVENT)event)
         {
            DPRINT("APP_EVENT_SW3_UP\n");
            set_proximity_level();
            // Set_Relay(3, 1);


          }
          if(APP_EVENT_SW4_DOWN == (APP_BUTTON_EVENT)event)
          {
             DPRINT("APP_EVENT_SW4_DOWN\n");
             set_proximity_level();
             createFrame(4);
            // handle_switch_event(4);
            // Set_Relay(4, 0);
             addr_config_mode = true;
             timeout_config_addr_CB();
          }
          if(APP_EVENT_SW4_UP == (APP_BUTTON_EVENT)event)
         {
            DPRINT("APP_EVENT_SW4_UP\n");
            set_proximity_level();
            // Set_Relay(4, 1);
          ...
          if(APP_EVENT_SW5_DOWN == (APP_BUTTON_EVENT)event)
          {
             DPRINT("APP_EVENT_SW5_DOWN\n");
             set_proximity_level();
             createFrame(5);
            // handle_switch_event(5);
            // Set_Relay(5, 0);
             addr_config_mode = true;
             timeout_config_addr_CB();
          }
          if(APP_EVENT_SW5_UP == (APP_BUTTON_EVENT)event)
         {
            DPRINT("APP_EVENT_SW5_UP\n");
            set_proximity_level();
            // Set_Relay(5, 1);

          }

          if(APP_EVENT_SW6_DOWN == (APP_BUTTON_EVENT)event)
          {
             DPRINT("APP_EVENT_SW6_DOWN\n");
             set_proximity_level();
             createFrame(6);
            // handle_switch_event(6);
            // Set_Relay(6, 0);
             addr_config_mode = true;
             timeout_config_addr_CB();
          }
          if(APP_EVENT_SW6_UP == (APP_BUTTON_EVENT)event)
         {
            DPRINT("APP_EVENT_SW6_UP\n");
            set_proximity_level();
            // Set_Relay(6, 1);
          }

          if(APP_EVENT_SW7_DOWN == (APP_BUTTON_EVENT)event)
          {
             DPRINT("APP_EVENT_SW7_DOWN\n");
             set_proximity_level();
             createFrame(7);
            // handle_switch_event(7);
            // Set_Relay(7, 0);
             addr_config_mode = true;
             timeout_config_addr_CB();
          }
          if(APP_EVENT_SW7_UP == (APP_BUTTON_EVENT)event)
            {
               DPRINT("APP_EVENT_SW7_UP\n");

               set_proximity_level();
              // Set_Relay(7, 1);
             }
          if(APP_EVENT_SW8_DOWN == (APP_BUTTON_EVENT)event)
          {
             DPRINT("APP_EVENT_SW8_DOWN\n");
             set_proximity_level();
             createFrame(8);
            // handle_switch_event(8);
           //  Set_Relay(8, 0);
             addr_config_mode = true;
             timeout_config_addr_CB();

         }
          if(APP_EVENT_SW8_UP == (APP_BUTTON_EVENT)event)
           {
              DPRINT("APP_EVENT_SW8_UP\n");
              set_proximity_level();
            //  Set_Relay(8, 1);
...
SetDefaultConfiguration(void)
{
 ...
 // appBinarySwitchSet(CMD_CLASS_BIN_OFF, 0, 0);
  DPRINT("---SetDefaultConfiguration---");
  ApplicationData.KeypadID = defined_keypadID = 2;
...
LoadConfiguration(void)
{
  ...
  if (ZPAL_STATUS_OK == status)
  {
    ...
    DPRINT(" in if ZPAL_STATUS_OK == status \r\n");
  ...
  {
    DPRINT("else Application FileSystem Verify failed\r\n");
...
readAppData(void)
{
  ...
  SApplicationData AppData;
   ...
   status= zpal_nvm_read(pFileSystemApplication, FILE_ID_APPLICATIONDATA, &AppData, sizeof(SApplicationData));
 // ASSERT(ZPAL_STATUS_OK == status); //Assert has been kept for debugging , can be removed from production code. This error hard to occur when a corresponing write is successfull
   ...
   if(ZPAL_STATUS_OK != status)
   {
    ...
    status = zpal_nvm_read(pFileSystemApplication, FILE_ID_APPLICATIONDATA, &AppData, sizeof(SApplicationData));
       ...
       defined_keypadID = AppData.KeypadID;
       DPRINTF("\n======Nvm3 App Read Data Success=======");
       DPRINTF("\nKeypad ID : %d\n", defined_keypadID);
   }
  return AppData;
}
...
//
//static bool switch_states[8] = { false, false, false, false, false, false,false,false };
//void update_switch_status(uint8_t switch_number, bool new_status)
//{
//    if (switch_number < 1 || switch_number > 8)
//        return; // Invalid switch number
//
//    switch_states[switch_number - 1] = new_status;
// //   Set_Led(switch_number, new_status ? 1 : 0);
//}
//
...
//    if (switch_states[switch_number - 1])
//    {
//         Set_Led(switch_number, 0); // Turn LED off
//       // Set_Relay(switch_number, 1);
//
//        // DPRINTF("Switch-%d is OFF..\n", switch_number);
//    }
//    else
//    {
//         Set_Led(switch_number, 1); // Turn LED on
//       // Set_Relay(switch_number, 0);
//
//        // DPRINTF("Switch-%d is ON..\n", switch_number);
//    }
//
//    // Toggle the state for the next button press
//    switch_states[switch_number - 1] = !switch_states[switch_number - 1];
//}
...
uint16_t GetConfigurationParameterValue(uint16_t parameterNumber)
{
  ...
  {
//    case 1:
//      return ApplicationData.source_main_val;
//    break;
//
//    case 2:
//      return ApplicationData.source_mid_val;
//    break;
////
//    case 3:
//      return ApplicationData.source_sub_val;
//    break;
//
//    case 4:
//      return ApplicationData.dest_main_val_1;
//    break;
//
//    case 5:
//      return ApplicationData.dest_mid_val_1;
//    break;
//
//    case 6:
//      return ApplicationData.dest_sub_val_1;
//    break;
//    case 7:
//      return ApplicationData.dest_main_val_2;
//    break;
//
//    case 8:
//      return ApplicationData.dest_mid_val_2;
//    break;
////
//    case 9:
//      return ApplicationData.dest_sub_val_2;
//    break;
//
//    case 10:
//      return ApplicationData.dest_main_val_3;
//    break;
//
//    case 11:
//      return ApplicationData.dest_mid_val_3;
//    break;
//
//    case 12:
//      return ApplicationData.dest_sub_val_3;
//    break;
//
//
//    case 13:
//      return ApplicationData.dest_main_val_4;
//    break;
//
//    case 14:
//      return ApplicationData.dest_mid_val_4;
//    break;
////
//    case 15:
//      return ApplicationData.dest_sub_val_4;
//    break;
//
//
//
//    case 16:
//      return ApplicationData.dest_main_val_5;
//    break;
//
//    case 17:
//      return ApplicationData.dest_mid_val_5;
//    break;
//
//    case 18:
//      return ApplicationData.dest_sub_val_5;
//    break;
//
//    case 19:
//      return ApplicationData.dest_main_val_6;
//    break;
//
//    case 20:
//      return ApplicationData.dest_mid_val_6;
//    break;
//
//    case 21:
//      return ApplicationData.dest_sub_val_6;
//    break;
//    case 22:
//      return ApplicationData.dest_main_val_7;
//    break;
//
//    case 23:
//      return ApplicationData.dest_mid_val_7;
//    break;
//
//    case 24:
//      return ApplicationData.dest_sub_val_7;
//    break;
//
//    case 25:
//      return ApplicationData.dest_main_val_8;
//    break;
//
//    case 26:
//      return ApplicationData.dest_mid_val_8;
//    break;
//
//    case 27:
//      return ApplicationData.dest_sub_val_8;
//    break;
//
//
//     default:
...
//     case 1:
//
//       ApplicationData.source_main_num = (uint16_t)parameterNumber;
//
//       ApplicationData.source_main_val = (uint16_t)decimalValue;  // Store as integer (numeric value)
//
//
//     break;
//
...
//       ApplicationData.source_mid_num = (uint16_t)parameterNumber;
//
//       ApplicationData.source_mid_val = (uint16_t)decimalValue; // Store as integer (numeric value)
//
//     break;
//
//    case 3:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.source_sub_num = (uint16_t)parameterNumber;
//
//      ApplicationData.source_sub_val =(uint16_t)decimalValue;
////      DPRINTF("ApplicationData: %d\n",ApplicationData.source_sub_val);
//
//    break;
//
//    case 4:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.dest_main_num_1 = (uint16_t)parameterNumber;
//
//      ApplicationData.dest_main_val_1 = (uint16_t)decimalValue;
...
//    case 5:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.dest_mid_num_1 = (uint16_t)parameterNumber;
//
//      ApplicationData.dest_mid_val_1 =(uint16_t)decimalValue;
//    break;
//
//    case 6:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.dest_sub_num_1 = (uint16_t)parameterNumber;
//
//      ApplicationData.dest_sub_val_1 = (uint16_t)decimalValue;
//    break;
//
//    case 7:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.dest_main_num_2 = (uint16_t)parameterNumber;
//
//      ApplicationData.dest_main_val_2 = (uint16_t)decimalValue;
////     DPRINTF("ApplicationData: %d\n",ApplicationData.dest_main_val);
//
//    break;
//
//    case 8:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.dest_mid_num_2 = (uint16_t)parameterNumber;
//
//      ApplicationData.dest_mid_val_2 =(uint16_t)decimalValue;
//    break;
//
//    case 9:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.dest_sub_num_2 = (uint16_t)parameterNumber;
//
//      ApplicationData.dest_sub_val_2 =(uint16_t)decimalValue;
//    break;
//
//    case 10:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.dest_main_num_3 = (uint16_t)parameterNumber;
//
//      ApplicationData.dest_main_val_3 = (uint16_t)decimalValue;
////     DPRINTF("ApplicationData: %d\n",ApplicationData.dest_main_val);
//
//    break;
//
//    case 11:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.dest_mid_num_3 = (uint16_t)parameterNumber;
//
//      ApplicationData.dest_mid_val_3 = (uint16_t)decimalValue;
//    break;
//
//    case 12:
////      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//      ApplicationData.dest_sub_num_3 = (uint16_t)parameterNumber;
//
//      ApplicationData.dest_sub_val_3 =(uint16_t)decimalValue;
//    break;
//
//
//
//    case 13:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_main_num_4 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_main_val_4 =(uint16_t)decimalValue;
// //     DPRINTF("ApplicationData: %d\n",ApplicationData.dest_main_val);
//
//     break;
//
//     case 14:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_mid_num_4 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_mid_val_4 =(uint16_t)decimalValue;
//     break;
//
//     case 15:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_sub_num_4 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_sub_val_4 = (uint16_t)decimalValue;
//     break;
//
//     case 16:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_main_num_5 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_main_val_5 =(uint16_t)decimalValue;
// //     DPRINTF("ApplicationData: %d\n",ApplicationData.dest_main_val);
//
//     break;
//
//     case 17:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_mid_num_5 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_mid_val_5 =(uint16_t)decimalValue;
//     break;
//
//     case 18:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_sub_num_5 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_sub_val_5 =(uint16_t)decimalValue;
//     break;
//
//     case 19:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_main_num_6 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_main_val_6 =(uint16_t)decimalValue;
// //     DPRINTF("ApplicationData: %d\n",ApplicationData.dest_main_val);
//
//     break;
//
//     case 20:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_mid_num_6 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_mid_val_6 =(uint16_t)decimalValue;
//     break;
//
//     case 21:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_sub_num_6 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_sub_val_6 =(uint16_t)decimalValue;
//     break;
//
//
//     case 22:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_main_num_7 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_main_val_7 =(uint16_t)decimalValue;
// //     DPRINTF("ApplicationData: %d\n",ApplicationData.dest_main_val);
//
//     break;
//
//     case 23:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_mid_num_7 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_mid_val_7 =(uint16_t)decimalValue;
//     break;
//
//     case 24:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_sub_num_7 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_sub_val_7 =(uint16_t)decimalValue;
//     break;
//
//     case 25:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_main_num_8 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_main_val_8 =(uint16_t)decimalValue;
// //     DPRINTF("ApplicationData: %d\n",ApplicationData.dest_main_val);
//
//     break;
//
//     case 26:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_mid_num_8 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_mid_val_8 =(uint16_t)decimalValue;
//     break;
//
//     case 27:
// //      DPRINTF("parameterNumber : %d, \n",parameterNumber);
//       ApplicationData.dest_sub_num_8 = (uint16_t)parameterNumber;
//
//       ApplicationData.dest_sub_val_8 =(uint16_t)decimalValue;
//     break;
  //  default:
...
//{
//  AppTimerRegister(&App_rx_Button_Timer, false, App_rx_Button_Timer_CB);
//
//
// }
