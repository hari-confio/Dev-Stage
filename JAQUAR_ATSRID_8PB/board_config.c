...
void read_btn_mcp_CB()
{
//  MCP23017_2_ReadInput_PinA(6);
//  MCP23017_2_ReadInput_PinA(7);
  uint8_t value = MCP23017_2_readReg(MCP23017_GPIO_A);
 // DPRINTF("GPIOA = 0x%02X\n", value);
//
  uint8_t A7 = (value >> 7) & 1;
  uint8_t A6 = (value >> 6) & 1;
//
 // DPRINTF("A7=%d, A6=%d\n", A7, A6);
  if(A7 == 0)//Button-7
    {
...
//  GPIO_PinModeSet(PORT_PB8, PIN_PB8, InputMode, 1);
  ...
  AppTimerRegister(&led_inc_toggle, false, led_inc_toggle_CB);
  AppTimerRegister(&read_btn_mcp, true, read_btn_mcp_CB);
  AppTimerRegister(&modbus_send_twice, false, modbus_send_twice_CB);
  GPIOINT_Init();
...
//  sw_8_cur_pos = GPIO_PinInGet(PORT_PB8,PIN_PB8);
  ...
  GPIOINT_CallbackRegister(PIN_PB1, Sw_interrupt_CB);

  GPIO_ExtIntConfig(PORT_PB2, PIN_PB2, PIN_PB2, true, true, true);
  GPIOINT_CallbackRegister(PIN_PB2, Sw_interrupt_CB);

  GPIO_ExtIntConfig(PORT_PB3, PIN_PB3, PIN_PB3, true, true, true);
  GPIOINT_CallbackRegister(PIN_PB3, Sw_interrupt_CB);

  GPIO_ExtIntConfig(PORT_PB4, PIN_PB4, PIN_PB4, true, true, true);
  GPIOINT_CallbackRegister(PIN_PB4, Sw_interrupt_CB);

  GPIO_ExtIntConfig(PORT_PB5, PIN_PB5, PIN_PB5, true, true, true);
  GPIOINT_CallbackRegister(PIN_PB5, Sw_interrupt_CB);

  GPIO_ExtIntConfig(PORT_PB6, PIN_PB6, PIN_PB6, true, true, true);
  GPIOINT_CallbackRegister(PIN_PB6, Sw_interrupt_CB);
